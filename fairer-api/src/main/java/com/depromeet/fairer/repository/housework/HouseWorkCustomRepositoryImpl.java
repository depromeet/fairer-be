        ...
        return jpaQueryFactory
                ...
                .where(member.memberId.eq(memberId).and(houseWork.scheduledDate.loe(date).and(houseWork.repeatEndDate.isNull().or(houseWork.repeatEndDate.goe(date))))
                        .and((houseWork.repeatCycle.eq(RepeatCycle.ONCE).and(houseWork.repeatPattern.eq(date.format(DateTimeFormatter.ofPattern("yyyy-MM-dd")))))
                            .or(houseWork.repeatCycle.eq(RepeatCycle.DAILY).and(repeatException.exceptionDate.isNull()))
                            .or(houseWork.repeatCycle.eq(RepeatCycle.WEEKLY).and(houseWork.repeatPattern.contains(date.getDayOfWeek().toString())).and(repeatException.exceptionDate.isNull()))
                            .or(houseWork.repeatCycle.eq(RepeatCycle.MONTHLY).and(houseWork.repeatPattern.castToNum(Integer.class).eq(date.getDayOfMonth())).and(repeatException.exceptionDate.isNull())))
                ).fetch();
